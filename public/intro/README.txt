...
- .jpg is expected. If you only have .png/.jpeg, either rename to .jpg or
  update the `src` paths in src/IntroSequence.jsx to match.
- Square-ish images look best (they are shown as polaroid squares), but any
  shape works — they are cropped to a square automatically.
- Until a file is present, a labelled placeholder shows in its place, so the
  app keeps working with or without the photos.
